...
void executarAnelAntihorario(MPI_Comm &new_comm, int nProcessos) {
    int ciclos = 0, op = 1;
    int rankGrupo;
    float MSG[2];
    ...
    FLAG=1;
    if (rankGrupo == 0) {
        cout << "Insira o valor para a comunicação anti-horária (y): ";
        cin >> VALOR;

        // Envia o valor inicial para o processo anterior e recebe o valor do processo seguinte
        MPI_Send(&MSG, 2, MPI_FLOAT, rankAnterior, 1, new_comm);
        MPI_Recv(&MSG, 2, MPI_FLOAT, rankSeguinte, 1, new_comm, MSI);
        ...
        while (op != 3) {
            ...
            switch (op) {
                case 1:
                    cout << "Insira o novo valor para a comunicação anti-horária (y): ";
                    cin >> VALOR;
                    MPI_Send(&MSG, 2, MPI_FLOAT, rankAnterior, 1, new_comm);
                    MPI_Recv(&MSG, 2, MPI_FLOAT, rankSeguinte, 1, new_comm, MSI);
                    ciclos++;
                ...
                case 2:
                    cout << "\n2 - CONTINUAR VOLTA\n" << endl;
                    VALOR--;
                    cout << "Valor de y decrementado | valor a ser enviado: " << VALOR << endl;
